...
import (
	...
	"github.com/kaigedong/go-video-website/video-server/api/defs"
	"github.com/kaigedong/go-video-website/video-server/utils"
)

func AddUserCredential(loginName string, pwd string) error {
	stmtIns, err := dbConn.Prepare("INSERT INTO users (login_name, pwd) VALUES (?, ?)") // 预编译，防止攻击
	if err != nil {
		return err
	}
...
func AddNewVideo(aid int, name string) (*defs.VideoInfo, error) {
	...
	t := time.Now()
	ctime := t.Format("Jan 02 2006, 15:04:05") // 这串是固定格式 M D Y, HH:MM::SS
